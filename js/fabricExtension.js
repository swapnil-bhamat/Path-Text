...
var renderedFontList = []; /* Rendered font list stores*/
var artifiSVGWidgetKey = "artifi-key"; /* We are use this key for storking widget key in SVG. That key we will replace in our service */
var artifiSVGWidgetId = "artifi-id"; /* We are use this key to store widget id*/
var artifiSVGshopbylogo_key = "shopbylogo_key"; /* We are use this key to store widget id*/
var artifiAlignMentKey = "artifi-text-align"; /* We are use this key to store widget id*/
...
fabric.util.object.extend(fabric.Text.prototype, /** @lends fabric.Path.prototype */ {
    ...
    _getFontDeclaration: function () {
        ...
        var fontIndex = renderedFontList.indexOf(fontFamily);
        // if (fontIndex == -1 && isFontAvailableOnLocal) {
        //     isFontAvailableOnLocal(fontFamily) ? renderedFontList.push(fontFamily) : (fontFamily = 'Arial');
        // }
        var fontDeclaration = [
...
fabric.util.object.extend(fabric.Image.prototype, /** @lends fabric.Path.prototype */ {
	...
	*/
    ...
    _render: function (ctx, noTransform) {
        try {
			...
			*/
            ...
            this.lockUniScaling = (this.PreserveWidgetAspectRatio != undefined) ? this.PreserveWidgetAspectRatio : true;
            if (this.PreserveImageAspectRatio) {
                this.drawImageForImageAspectRatio(ctx, imageElement, noTransform);
            } else {
                this.drawImageOnContext(ctx, imageElement, noTransform, this.width, this.height);
            }
    ...
    drawImageOnContext: function (ctx, imageElement, noTransform, width, height) {
        imageElement &&
            ctx.drawImage(
                imageElement,
    ...
    drawImageForImageAspectRatio: function (ctx, imageElement, noTransform) {
        // var editorModel = main.getModel(Artifi.ModelConstant.EDITOR_MODEL);
        // editorModel.getUpdatedPreserveImageDimention(this);
        this.drawImageOnContext(ctx, imageElement, noTransform, this.imageCurrentWidth, this.imageCurrentHeight);
    },
	 ...
	 */
    toSVG: function (reviver, hasOverlayImage) {
        var markup = this._createBaseSVGMarkup(), x = -this.width / 2, y = -this.height / 2;
        if (this.PreserveImageAspectRatio) {
            x = -this.imageCurrentWidth / 2; y = -this.imageCurrentHeight / 2;
        }
        if (this.group && this.group.type === 'path-group') {
            x = this.left;
            y = this.top;
        }
        var isEmbossDeboss = this.isEffectApplied("EmbossDeboss");
        var isEmbroideryOrEmbossDebossApplied = this.isEmbroideryOrEmbossDebossApplied();
        markup.push(
            ...
            '" width="', this.PreserveImageAspectRatio ? this.imageCurrentWidth : this.width,
            '" height="', this.PreserveImageAspectRatio ? this.imageCurrentHeight : this.height,
            '" originalFileName="', this.GetFilename(this.originalUrl),
            '" preserveAspectRatio="none"',
            (isEmbossDeboss ? ' embossDeboss="' + this.GetembossDeboss() + '" ' : ''),
            (isEmbroideryOrEmbossDebossApplied ? ' effectUrl="' + this.src + '" ' : ''),
            hasOverlayImage ? ' isOverlayImage="true"' : '',
            /*--------------Code Start -----------------*/
            /* SVG Artifi keys use for replacing the value of this keys in c# services**/
            (this.widget_key ? ' ' + artifiSVGWidgetKey + '="' + this.widget_key + '" ' : ''),
            (this.id ? ' ' + artifiSVGWidgetId + '="' + this.id + '" ' : ''),
            (this.shopbylogo_key ? ' ' + artifiSVGshopbylogo_key + '="' + this.shopbylogo_key + '" ' : ''),
            /*--------------Code End -----------------*/
            '></image>\n'
        );
    ...
    getImageSrcForSvg: function () {
        var _this = this;
        // var printImage = this.isPrintImageUrlPresent() ? null : this.get('printImage');
        // var imageSrc = null;
        ...
        // }
        // if (!imageSrc) {
            // var src = this.isEmbroideryOrEmbossDebossApplied() ? this.originalUrl : this.src;
            // /*use this.src to get the actual image src rather than base 64*/
            // imageSrc = this.removeAPICallFromURL(src);
            ...
            // if (Artifi.Config.isUsedPlaceHolderWithoutborder && imageSrc.indexOf(Artifi.Config.artifiPlaceHolderImageName) > -1) {
            //     imageSrc = imageSrc.replace(Artifi.Config.placeHolderImageUrl, Artifi.Config.placeHolderImageUrlWithoutBorder);
            // }

        //}

        return this.removeAndAddNewProtocal(this.src);
    },

    isEmbroideryOrEmbossDebossApplied: function () {
        return this.isEffectApplied("EmbossDeboss") || this.isEffectApplied("Embroidery");
    },

    isPrintImageUrlPresent: function () {
        try {
            // var editorModel = main.getModel(Artifi.ModelConstant.EDITOR_MODEL);
            // if (editorModel) {
            //     return editorModel.isPrintImageUrlPresent(this.id, this.viewId);
            // }
        } catch (err) {
            return false;
        }
    ...
    /* This function is used to get the company id to exclude effects in in Print PDF*/
    isExcludeInPrintPDF: function () {
        // var appModel = main.getModel(Artifi.ModelConstant.APPLICATION_MODEL);
        // var companyId = appModel.getCompanyId();
        // var excludedCompIds = Artifi.Config.exludeEffectsInPrintPdf;
        // return excludedCompIds.indexOf(companyId) >= 0;
        return true;
    },
    /* This function is used to get the flag of emb deb if return true and false accordingly */
    isEffectApplied: function (effect) {
        var embDebFlag = false;
        if (this.customFilters && this.customFilters.length > 0) {
            for (var i = 0; i < this.customFilters.length; i++) {
                var filter = this.customFilters[i];
                var filterType = filter && filter.type ? filter.type.toLowerCase() : null;
                if (filterType == effect.toLowerCase()) {
                    embDebFlag = true;
                }
            }
        }
    ...
    /* This function is used to set the embosed debosed effect in svg*/
    GetembossDeboss: function () {
        var embDebVal = '';
        if (this.customFilters && this.customFilters.length > 0) {
            for (var i = 0; i < this.customFilters.length; i++) {
                var filter = this.customFilters[i];
                var filterType = filter && filter.type ? filter.type.toLowerCase() : null;
                if (filterType == "EmbossDeboss".toLowerCase()) {
                    embDebVal = filter.subType;
                }
            }
        }
        return embDebVal;
	...
	*/
    ...
    removeThumbnailSrc: function (src) {
        ...
        if (isOutputSrc) {
            ...
            newSrc = newSrc.replace("\/Standard\/", "\/Original\/");
        } else if (isThumbSrc) {
            newSrc = src.replace("\/Thumbnail\/", "\/Original\/");
        } else if (isStdSrc) {
    ...
    toObject: function (propertiesToInclude) {
        var NUM_FRACTION_DIGITS = fabric.Object.NUM_FRACTION_DIGITS,
            ...
            object = {
                ...
                shopbylogo_key: this.shopbylogo_key,
                WidgetBorder: this.WidgetBorder,
                Rotate180Degree: this.Rotate180Degree,
                src: (this._originalElement) ? this._originalElement.src || this._originalElement._src : "",
                filters: this.filters.map(function (filterObj) {
                    return filterObj && filterObj.toObject();
                }),
                crossOrigin: this.crossOrigin,
                PreserveWidgetAspectRatio: this.PreserveWidgetAspectRatio,
                PreserveImageAspectRatio: this.PreserveImageAspectRatio,
                IsVisibleOnOutput: this.IsVisibleOnOutput
            };
        this.textSrc ? (object.textSrc = this.textSrc) : null;
        this.textSrcLeft ? (object.textSrcLeft = this.textSrcLeft) : null;
        this.textSrcTop ? (object.textSrcTop = this.textSrcTop) : null;
        if (this.PreserveImageAspectRatio) {
            this.imageCurrentWidth = this.imageCurrentWidth;
            this.imageCurrentHeight = this.imageCurrentHeight;
        }
...
fabric.util.object.extend(fabric.StaticCanvas.prototype, /** @lends fabric.Path.prototype */ {
   ...
   */
    toSVG: function (options, reviver) {
        ...
        if (this.maskData && this.maskData.maskData) {
            var maskShapeMarkup = this.maskData.maskData.toSVG();
            var maskString = '<clipPath xmlns="http://www.w3.org/2000/svg" id="linkShape">';
            maskString += maskShapeMarkup;
            maskString += '</clipPath> <g style="clip-path:url(#linkShape);">'
            markup.push(maskString);
        }
    ...
    _setSVGObjects: function (markup, reviver) {
        ...
        var objectsList = this.getObjects();
        for (var i = 0, objects = objectsList, len = objects.length; i < len; i++) {
            if (typeof objects[i].IsVisibleOnOutput == 'undefined' || objects[i].IsVisibleOnOutput) {
                if (objects[i].type == 'CustomPath' && this.isPathAllowedInOutput(objects[i], objectsList)) {
                    markup.push(objects[i].toSVG(reviver));
                } else if (objects[i].type !== 'CustomPath') {
                    markup.push(objects[i].toSVG(reviver));
                }
            }
        }
	...
	/**
	 * Function checks wheather custom path allowed in output or not
	 * @param1 {customPathObj}
	 * @param2 {objectsList}
	 */
    isPathAllowedInOutput: function (customPathObj, objectsList) {
        for (var index = 0, objects = objectsList, len = objects.length; index < len; index++) {
            if (objects[index].id == customPathObj.linkId) {
                return objects[index].showPath;
            }
        }
        return false;
    },
	...
	*/
    ...
    __serializeBgOverlay: function () {
        ...
        if (this.overlayImage) {
            ...
            data.overlayImage.fileName = this.overlayImage.fileName ? this.overlayImage.fileName : "";
        }
        /* ==================================== Custom Changes for Artifi Project ==================== */
        if (this.maskData) {
	...
	*/
    _setSVGBgOverlayImage: function (markup, property) {
        if (this[property] && this[property].toSVG) {
            markup.push(this[property].toSVG("", true));
        }
    },



    /* TOOLBAR FUNCATANALITY STARTS */
    ...
    sendToBack: function (object) {
        ...
        if (object.type == "circle-text") {
            var customPathObject = this._getCustomPathObject(object.id);
            this._placedObjAtBottom(customPathObject);
        }
        this._placedObjAtBottom(object);
        return this.renderAll && this.renderAll();
    },

	/*
	* function is used to placed object at the bottom
	*/
    _placedObjAtBottom: function (object) {
    ...
    bringToFront: function (object) {
        var idx = this._objects.indexOf(object);
        this._placeObjAtTop(object);
        if (object.type == "circle-text") {
            var customPathObject = this._getCustomPathObject(object.id);
            this._placeObjAtTop(customPathObject);
        }
        return this.renderAll && this.renderAll();
    },

	/*
	* fucntion is used to placed object at the top
	*/
    _placeObjAtTop: function (object) {
        fabric.util.removeFromArray(this._objects, object);
    ...
    sendBackwards: function (object, intersecting) {
        ...
        if (idx !== 0) {
            var newIdx = this._getNewLowerIndex(idx);

            newIdx = this._getNewIndexForSendBackwrd(object, newIdx);

            this._moveObjAtIndex(newIdx, object);

            if (object.type == "circle-text") {
                var customPathObject = this._getCustomPathObject(object.id);
                this._moveObjAtIndex(newIdx + 1, customPathObject);
            }
        }
        return this.renderAll && this.renderAll();
    },
	...
	/*
	* function is used to set new index for circle text for send backwrd action
	* @param1 {object} : contains active object
	* @param2 {newIdx} : contains new id
	*/
    _getNewIndexForSendBackwrd: function (object, newIdx) {
        if (object.type == "circle-text") {
            /*if new id is, custom path , means it is going to be below existing circle text*/
            if (this._objects[newIdx].type == "CustomPath") {
                newIdx -= 1;
            }
        } else if (object.type != "circle-text" && (this._objects[newIdx].type == "CustomPath")) {
            newIdx -= 1;
        }
        return newIdx;
    },
	...
	/*
	* function gives custom path object for provided circle text id
	*/
    _getCustomPathObject: function (circleTextId) {
        for (var index in this._objects) {
            if (this._objects[index].id == (circleTextId + '_customPath')) {
	...
	/*
	* @param1 {idx} : contains position/index at which object have to place
	* @param2 {object} : contains object which needs to place at idx index
	* fuction used to place object at given index
	*/
    _moveObjAtIndex: function (idx, object) {
        fabric.util.removeFromArray(this._objects, object);
        this._objects.splice(idx, 0, object);
    },

	/*
	* function is used to dive lower index by provided idx, if any lower index object is present
	*/
    _getNewLowerIndex: function (idx) {
    ...
    bringForward: function (object, intersecting) {
        ...
        if ((idx !== this._objects.length - 1) && !this._isCircleTextOnTop(object, idx)) {

            var newIdx = this._getNewUpperIndex(idx);
            newIdx = this._getNewIndexForBringForwrd(object, newIdx);
            this._moveObjAtIndex(newIdx, object);
	...
	/*
	* function is used to get circle text object new index, and sets its custom path to its position
	* @param1 {object} : contains active object
	* @param2 {newIdx} : contains new id
	*/
    _getNewIndexForBringForwrd: function (object, newIdx) {
        if (object.type == "circle-text") {
            /*if new id is, custom path , means it is going to be below existing circle text*/
            if (this._objects[newIdx + 1] && this._objects[newIdx + 1].type == "circle-text") {
                newIdx += 1;
            }

            var customPathObject = this._getCustomPathObject(object.id);
            this._moveObjAtIndex(newIdx + 1, customPathObject);
        } else if (this._objects[newIdx].type == "circle-text") {
            newIdx += 1;
        }
        return newIdx;
    },
	...
	/*
	* function checks wheather provided object is circle text and is it on the top
	* if yes, then return true else false
	*/
    _isCircleTextOnTop: function (object, idx) {
        return (object.type == "circle-text" && (idx == this._objects.length - 2)) ? true : false;
    },
	/*
	* function return circle text "custom path" object
	* @param {circleTextId} : contains circle text id
	*/
    _getCustomCircleTextObject: function (circleTextId) {
        for (object in this._objects) {
            if (object.id == circleTextId + '_customPath') {
                return object
            }
        }
        return null;
	...
	/*
	* function gives provided index, one upper index, if upper object is present 
	*/
    _getNewUpperIndex: function (idx) {
        return (idx < this._objects.length) ? (idx + 1) : idx;
    }



    /* TOOLBAR FUNCATANALITY ENDS */
});
/* ====  Static Canvas End  =====*/
...
*/
    stateProperties: (
        ...
        /* ======================== Custom Changes for Artifi ==========================*/
        'widgetType widgetSubtType id originalWidth originalHeight imageUniqueName ImageUniqueNameOnUGC originalUrl scaleFactor cropArea  customFilters currentWidgetWidth currentWidgetHeight decalId libProp,caption,widget_key,group_key,price_key,shopbylogo_key'
    ).split(' '),
...
(function (global) {
    ...
    fabric.Textbox = fabric.util.createClass(fabric.Text, {
        ...
        initialize: function (objects, options) {
            ...
            this.setCustomFilter(options.customFilters);
        },
		/* Added check of customFilters if available the assign in widget data
		...
		*/
        _set: function (prop, value) {
            ...
            if (prop == 'customFilters') {
                this.setCustomFilter(value);
            }
            return this;
        },
        setCustomFilter: function (customFilters) {
            // if (customFilters) {
            //     for (var i = 0; i < customFilters.length; i++) {
            //         var filter = customFilters[i];
            //         var filterType = filter && filter.type ? filter.type.toLowerCase() : null;
            //         if (filterType == Artifi.ApplicationConstant.EMBROIDERY.toLowerCase() ||
            //             filterType == Artifi.ApplicationConstant.TEXTURE.toLowerCase() ||
            //             filterType == Artifi.ApplicationConstant.EMBOSS_DEBOSS.toLowerCase()) {
            //             this.textImageRenderer = new fabric.TextEmbroidery(filter);
            //         }
            //         else {
            //             this.embossDboss = "";
            //         }
        ...
        toObject: function (propertiesToInclude) {
            return fabric.util.object.extend(this.callSuper('toObject', propertiesToInclude), {
                ...
                customFilters: this.getCustomFilters ? this.getCustomFilters(this.customFilters) : []

            });
        },
        getCustomFilters: function (customFilters) {
            var finalCustomFilter = [];
            if (customFilters) {
                for (var i = 0; i < customFilters.length; i++) {
                    var filter = customFilters[i];
                    if (filter.type == "Embroidery" || filter.type == "Texture" || filter.type == "EmbossDeboss") {
                        this.textImageRenderer ? finalCustomFilter.push(this.textImageRenderer.toObject()) : null;
                    }
                }
            }
            return finalCustomFilter;
        },
		...
		*/
        ...
        _getCSSValue : function (element, property) {
      ...
      */
        _measureTextMetrics: function (ctx, textstring) {
            ...
            // then we try to get the real value from the browser
            var leadDivHeight = this._getCSSValue(leadDiv, "height");
            leadDivHeight = leadDivHeight.replace("px", "");
            if (leadDivHeight >= fontSize * 2) { metrics.leading = (leadDivHeight / 2) | 0; }
            document.body.removeChild(leadDiv);
        ...
        _getHeightOfLine: function (ctx, lineIndex, textLines) {
            ...
            //console.log(this._measureTextMetrics(ctx, textLines[lineIndex]));
        ...
        toSVG: function () {
            ...
            //var updatedfont = Artifi.Utils.cloneObject(this.fontSize);
            // try {
            //     var applicationModel = main.getModel(Artifi.ModelConstant.APPLICATION_MODEL);
            //     var companyId = applicationModel.getCompanyId();
            //     var productCode = applicationModel.appData.productCode;
            //     if (companyId && ($.inArray(companyId, Artifi.Config.allowedCompaniesForFontSizeChange) != -1) && productCode == Artifi.Config.wristbandOnePageproductCode) {
            //         updatedfont = Artifi.Utils.cloneObject(this.fontSize) / Artifi.Config.fontSizePdfRatio;
            //     }
            // }
            // catch (e) {
            ...
            //To adjust position correctly
            //offsets.textTop -= 2;
            var textSrc = this.textSrc;
            textSrc = textSrc ? textSrc.replace("Output", "TextOrigionalImage") : this.textSrc;

            var textImagePositions = (this.textImageElement || this.textSrc) ? this.getTextImgPosition() : null;


            textSrc = textSrc ? textSrc.replace("Thumbnail", "Standard") : this.textSrc;
            return [
                ...
                (this.fontSize ? 'font-size="' + updatedfont + '" ' : ''),
                (this.fontStyle ? 'font-style="' + this.fontStyle + '" ' : ''),
                (this.fontWeight ? 'font-weight="' + this.fontWeight + '" ' : ''),
                (this.textSrc ? 'text-src="' + textSrc + '" ' : ''),
                (textImagePositions ? 'text-SrcLeft="' + textImagePositions.X + '" ' : ''),
                (textImagePositions ? 'text-SrcTop="' + textImagePositions.Y + '" ' : ''),
                (this.isEffectApplied("EmbossDeboss") ? ' embossDeboss="' + this.GetembossDeboss() + '" ' : ''),
                /*--------------Code Start -----------------*/
                /* SVG Artifi keys use for replacing the value of this keys in c# services**/
                (this.widget_key ? artifiSVGWidgetKey + '="' + this.widget_key + '" ' : ''),
                (this.id ? artifiSVGWidgetId + '="' + this.id + '" ' : ''),
                (this.shopbylogo_key ? artifiSVGshopbylogo_key + '="' + this.shopbylogo_key + '" ' : ''),
                (artifiAlignMentKey + '="' + this.textAlign + '" '),
        ...
        getTextImgPosition: function () {
            return { X: this.getXPosition(), Y: this.getYPosition() }
        ...
        getXPosition: function () {
            switch (this.textAlign) {
                case "left":
                    return (this.textSrcLeft);
                case "center":
                    return (this.textSrcLeft + ((this.width) / 2));
                case "right":
                    return (this.textSrcLeft + this.width);
            }
        },
        ...
        getYPosition: function () {
            switch (this.vAlign) {
                case "top":
                    return -((this.height + 50) / 2); break;
                case "middle":
                    return this.textSrcTop;
                case "bottom":
                    return -this.height / 2 - ((this.height - 50 - this.height) / 2); break;
            }
        },


        /* This function return flag for embossed or debossed assign to widget */
        isEffectApplied: function (effect) {
            var embDebFlag = false;
            if (this.customFilters && this.customFilters.length > 0) {
                for (var i = 0; i < this.customFilters.length; i++) {
        ...
        /* This function return the embossed debossed value if assign to widget */
        GetembossDeboss: function () {
            var _this = this;
            var embDebVal = '';
            if (this.customFilters && this.customFilters.length > 0) {
        ...
        /* This function is used to get Svg style */
        getSvgStyles: function () {
            var fill = this.fill
                ? (this.fill.toLive ? 'url(#SVGID_' + this.fill.id + ')' : this.fill)
                : 'none',
            ...
            function renderLinesAtOffset(offset) {
                for (var i = 0, len = textLines.length; i < len; i++) {
                    ...
                    ctx.fillRect(
                        ...
                        top, lineWidth, _this.fontSize / 15);
	   ...
	   */
        ...
        _drawChars: function (method, ctx, chars, left, top) {
            if (method == "fillText" && this.textImageElement && this.textImageElement.src) {
                this.textImageRenderer.drawTextImage(this, method, ctx, chars, left, top);
            }
            else {
	...
	/*
	 * if text object contains its image src, then object will load async else Synchronous loaded object
	 */
    fabric.Textbox.fromObject = function (object, callback) {
        ...
        if (instance && instance.textSrc) {
            loadTextImageElement(instance, callback);
        } else {
            return instance;
        }
    };

    function loadTextImageElement(instance, callback) {
        fabric.util.loadImage(instance.textSrc, function (img) {
            instance.textImageElement = img;
...
(function (global) {
    ...
    fabric.PathText = fabric.util.createClass(fabric.Text, {
		...
		*/
        ...
        _boundaries: [],
        CanGrow: false,
        enablePadding: true,/* set enablePadding = true to set padding between text and path of circle*/
		...
		*/
        _createPathObject: function (options) {
            ...
            pathOptions['opacity'] = (typeof options['showPath'] !== "undefined" && options['showPath']) ? 1 : 0.01;
        ...
        _set: function (prop, value) {
            if (prop == 'pathString' && typeof value != 'object' && this.canvas) {
                ...
                pathOptions['opacity'] = (typeof this.textPath['opacity'] !== "undefined") ? this.textPath['opacity'] : 0.01;
                options.textPath = new fabric.CustomPath(value, pathOptions);
                if (!this.showPath) {
                    options.textPath.padding = this.enablePadding ? this.currentHeight / 2 : 0;
                    options.textPath.strokeWidth = this.nonScaledFontSize / this.scaleX;
                }
                else {
                    options.textPath.strokeWidth = 1 / this.scaleX;
                    options.textPath.padding = this.enablePadding ? this.currentHeight : 0;
                    this.canvas.targetFindTolerance = this.currentHeight;
                }
                ...
                if (this.showPath) {
                    this.enablePadding ? this.textPath.set('padding', this.currentHeight) : this.textPath.set('padding', 0);
                    this.canvas.targetFindTolerance = this.currentHeight / 2;
                }
                this.textPath.set(prop, value);
                this.textPath.setCoords();
                if (prop != 'hasControls' && prop != 'hasBorders') {
                    this.callSuper('_set', prop, value);
                }
            }
            ...
            */
                ...
                if (value) {
                    ...
                    this.textPath.set('opacity', 0.01);
                }
                this.callSuper('_set', prop, value);
            }
            else if (prop == 'CanGrow') {
                this.callSuper('_set', prop, value);
                if (!value) {
                    var relativeFontSize = this.nonScaledFontSize ? this.nonScaledFontSize : this.fontSize;
                    this._updateNonScaledFontSize(relativeFontSize);
                }
            }
            else if (prop == 'nonScaledFontSize' && this.canvas) {
                this._updateNonScaledFontSize(value);
            }
		...
		/**
		* @private
		* @param {nonScaledFontSizeValue} Non-scaled font size value.
		*/
        _updateNonScaledFontSize: function (nonScaledFontSizeValue) {
            this.callSuper('_set', 'nonScaledFontSize', nonScaledFontSizeValue);
            var fontSizeValue = this.getCalculatedFontSize(nonScaledFontSizeValue);
		...
		*/
        ...
        _renderLetterAtPoint: function (method, ctx, letter, point, top, lineIndex, halfNonTransformedHeight, summedLineHeight, style, approximating) {
            ...
            if ((point && this.wantTextPathWithLessOverlap && !point.outOfPath) || (point && this.removeExtraChar && !point.outOfPath)) {
                ...
                if (!fillTextButNoFillDefinition && !strokeTextButNoStrokeDefinition) {
                    ...
                    /*vertical offset calculated for vertical alignment*/
                    var verticalLetterOffset;
                    if (this.vAlign == 'middle') {
                        verticalLetterOffset = (3 * halfHeightOfLetter) / 4;
                    }
                    else if (this.vAlign == 'top') {
                        verticalLetterOffset = 0;
                    }
                    else if (this.vAlign == 'bottom') {
                        verticalLetterOffset = (5 * halfHeightOfLetter) / 4;
                    }
                    ...
                    ctx[method](letter, left, verticalLetterOffset);
                    ctx.translate(-adjustmentToContextDrawing, 0);
                }
        ...
        getRunningDistanceAndMode: function (line, ctx) {
            ...
            if (this.textAlign === "center") {
                ...
                var lineWidth = ctx.measureText(line).width;
                startingDistance = (((maximumConsumedDistance - lineWidth) / 2) < 0) ? 0 : (maximumConsumedDistance - lineWidth) / 2;
            ...
            return { runningDistance: runningDistance, reverseMode: reverseMode, text: line };
        },

        getCalculatedFontSize: function (currentFontSize) {
            var fontSize = currentFontSize || this.fontSize;
            // var editorModel = main.getModel(Artifi.ModelConstant.EDITOR_MODEL);
            // var relativeFontSize = this.originalFontSize;
            // if (editorModel.isPrintAreaScalingAllowed() && this.nonScaledFontSize) {
            //     relativeFontSize = this.nonScaledFontSize;
            // }
        ...
        resetTransformTool: function () {
            if (!this.showPath) {
                this.textPath.padding = this.enablePadding ? this.currentHeight / 2 : 0;
                this.textPath.strokeWidth = this.nonScaledFontSize / this.scaleX;
            }
            else {
                this.textPath.strokeWidth = 1 / this.scaleX;
                this.textPath.padding = this.enablePadding ? this.currentHeight : 0;
            }
            this.textPath.setCoords();
        },
        ...
        getAllLettersPoint: function (linePoints, breakWhenOutOfPath) {
            ...
            var ctx = linePoints.ctx;
            var runningDistanceAndModeObj = this.getRunningDistanceAndMode(line, ctx);
            var runningDistance = runningDistanceAndModeObj.runningDistance;
            var reverseMode = runningDistanceAndModeObj.reverseMode;
            line = runningDistanceAndModeObj.text;
            ...
            var pathLength = this.textPath.getSVGPathElement().getTotalLength();
            var textLength = ctx.measureText(linePoints.line).width;
            if ((textLength > pathLength) && breakWhenOutOfPath) {
                isOutOfpath = true;
            }
        ...
        getAllowedSmallFontSizes: function (currentFontSize) {
            if (this.availableFontSizes.length == 0) {
                return currentFontSize;
            }
            var allowedSmallFontSizes = [];
            for (var index = 0; index <= this.availableFontSizes.length; index++) {
                if (parseFloat(this.availableFontSizes[index]) < parseFloat(currentFontSize)) {
                    allowedSmallFontSizes.push(parseFloat(this.availableFontSizes[index]));
                }
            }
            allowedSmallFontSizes.reverse();
            allowedSmallFontSizes = (allowedSmallFontSizes.length == 0) ? parseFloat(this.availableFontSizes[0]) : allowedSmallFontSizes;
        ...
        getAllowedBiggerFontSize: function (currentFontSize) {
            ...
            for (var index = 0; index <= this.availableFontSizes.length; index++) {
                if (parseFloat(this.availableFontSizes[index]) > parseFloat(currentFontSize)) {
                    allowedBiggerFontSizes.push(parseFloat(this.availableFontSizes[index]));
                }
            }
            allowedBiggerFontSizes = (allowedBiggerFontSizes.length == 0) ? parseFloat(this.availableFontSizes[this.availableFontSizes.length - 1]) : allowedBiggerFontSizes;
        ...
        _renderTextLineOnTextPath: function (method, ctx, line, left, top, lineIndex, spaceWidth) {
            ...
            thisLineMetaData.letters = this.getFilteredObjects(thisLineMetaData.letters);
            var thisLineMetaData = this._boundaries[lineIndex];
            // Obtain the height of the untransformed bounding box that the unpathed text would have created.
            var nonTransformedHeightOfAllLines = this._getObservedTotalLineHeight(ctx);
            var halfNonTransformedHeight = nonTransformedHeightOfAllLines / 2;
            ...
            for (var charIndex = 0, len = thisLineMetaData.letters.length; charIndex < len; charIndex++) {
                ...
                if (!charExceed && letterEntry && letterEntry.point.outOfPath) {
                    charExceed = true;
                }
                else if (letterEntry && !letterEntry.point.outOfPath) {
		...
		/*
		*  function is used to filter objects for undefined and null conditions 
		*/
        getFilteredObjects: function (objectsList) {
            return objectsList.filter(Boolean);
        },
		/**
		 * Returns object representation of an instance
		 * @param {Array} [propertiesToInclude] Any properties that you might want to additionally include in the output
        ...
        getSvgStyles: function () {
            ...
            //if embose debose fill=blacke else
            var fill = this.fill
                ? (this.fill.toLive ? 'url(#SVGID_' + this.fill.id + ')' : this.fill)
                : 'none',
                fillRule = (this.fillRule === 'destination-over' ? 'evenodd' : this.fillRule),
        ...
        _wrapSVGTextAndBg: function (markup, textAndBg, shadowSpans, offsets) {
            ...
            var relativeFontSize = this.CanGrow ? this.nonScaledFontSize : this.originalFontSize;
            // var editorModel = main.getModel(Artifi.ModelConstant.EDITOR_MODEL);
            // if (editorModel.isPrintAreaScalingAllowed() && this.nonScaledFontSize) {
            //     relativeFontSize = this.nonScaledFontSize;
            // }
            ...
            markup.push(
                ...
                (relativeFontSize ? 'font-size="' + relativeFontSize + '" ' : ''),
                (this.fontStyle ? 'font-style="' + this.fontStyle + '" ' : ''),
                (this.fontWeight ? 'font-weight="' + this.fontWeight + '" ' : ''),
                (this.textDecoration ? 'text-decoration="' + this.textDecoration + '" ' : ''),
                'style="', this.getSvgStyles(), '" ',
		...
		*/
        _setSVGTextLineText: function (textLine, i, textSpans, lineHeight, lineTopOffsetMultiplier) {
            ...
            if (this.vAlign == 'top') {
                ...
                lineTopOffset = (this._lastBoundaries && this._lastBoundaries[i]) ? toFixed((this._lastBoundaries[i].letters[i].point.halfHeightOfLetter / 2 + this._lastBoundaries[i].letters[i].point.halfHeightOfLetter / 4) * this.scaleX, 2) : 0;
            }
            else if (this.vAlign == 'bottom') {
                lineTopOffset = (this._lastBoundaries && this._lastBoundaries[i]) ? toFixed((this._lastBoundaries[i].letters[i].point.halfHeightOfLetter + this._lastBoundaries[i].letters[i].point.halfHeightOfLetter / 4) * this.scaleX, 2) : 0;
            }
...
//         this._controlsVisibility.mtr = true;
//         this._controlsVisibility.mt = false;
...
//         }
//     }
//     else if (!this.get('lockUniScaling')) {
//         this._controlsVisibility.mtr = false;
//         this._controlsVisibility.mt = true;
//         this._drawControl('mt', ctx, methodName,
//             left + width / 2 - scaleOffset + customPadding,
//             top - scaleOffset - padding + customPadding, customSize);
//     }
...
/*
 * function is used to get object base 64 data
 */
fabric.Object.prototype.objectToDataURL = function (options) {
    ...
    el.width = this.currentWidth + 50;
    el.height = this.currentHeight + 50;
...
/*
* Extended : Becous As text emrboidery effect contains image , so text should also load asyn when embroiedry is applied 
*/
fabric.util.enlivenObjects = function (objects, callback, namespace, reviver) {
    ...
    objects.forEach(function (o, index) {
        ...
        if (klass.async || isTextRemoteEffectApplied(o)) {
...
/*Before scaling or rotating widget, trigger event*/
fabric.Canvas.prototype.onBeforeScaleRotate = function () {
    this.fire('object:beforeScaleRotate');
}
/*Before scaling or rotating widget, trigger event*/
fabric.Canvas.prototype.onBeforeScaleRotate = function () {
    this.fire('object:beforeScaleRotate');
...
/*
* For Text Widgets : Check embroidery is applied , if yes then load asyn
*/
isTextRemoteEffectApplied = function (object) {
    var isTextRemoteEffectApplied = false;
    if (object.type == "textbox") {
        var customFilters = object.customFilters || [];
        customFilters.forEach(function (effectObj, index) {
            if ((effectObj.type == "Embroidery" || effectObj.type == "EmbossDeboss") && object.textSrc) {
                isTextRemoteEffectApplied = true; return false;
            }
        });
    }
    return isTextRemoteEffectApplied;
}
/*
